...
class BM25:
    ...
    def createQueryFrequencyDict(self):
        ...
        terms = [x for x in terms if x not in self.stop_words]
        for term in terms:
    ...
    def printScores(self, qId):
        ...
        file = open("BM_Output_Stopping/"+str(qId)+"-score.txt", "w")
        rank = 0
        for tup in sortedDict:
            rank += 1
            file.write(str(qId) + " Q0 " + str(tup[0]) + " " + str(rank) + " " + str(tup[1]) + " BM25Stopping\n")
            if rank == 100:
                break
