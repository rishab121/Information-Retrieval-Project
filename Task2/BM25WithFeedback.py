...
from PsuedoRelevanceFeedBack import PsuedoRelevanceFeedBack
...
class BM25WithFeedback:
    ...
    def printScores(self, qId):
        ...
        file = open("BM_OutputWithFeedback/"+str(qId)+"-withFeedbackScore.txt", "w")
        rank = 0
    ...
    def main(self):
        ...
        for q in queries.keys():
            ...
            psuedoRelevanceFeedBack = PsuedoRelevanceFeedBack()
            queryAr.extend(psuedoRelevanceFeedBack.main(q, queryAr))
            self.queryFrequencyDict = defaultdict(int)
            self.docScoreDict = defaultdict(int)
