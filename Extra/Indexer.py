...
class SimpleIndexer:
    ...
    def sort_inverted_index(self, file_name, inverted_index):
        ...
        if file_name == 'unigram_with_position.txt':
            print "in here"
            self.save_position_to_file(file_name, sorted_inverted_index)
        else:
    ...
    def create_unigram_with_position(self):
        for doc_id in self.doc_words.keys():
            words = self.doc_words[doc_id]
            length = len(words)
            for i in range(0, length):
                word = words[i]
                inverted_list_term = self.unigram_inverted_index_with_positions[word]
                if len(inverted_list_term.keys()):
                    if doc_id in inverted_list_term.keys():
                        inverted_list_term[doc_id][0] += 1
                        inverted_list_term[doc_id][1].append(i)
                    else:
                        inverted_list_term[doc_id] = [1, [i]]
                else:
                    inverted_list_term[doc_id] = [1, [i]]
                    self.unigram_inverted_index_with_positions[word] = inverted_list_term
        self.sort_inverted_index(
            'unigram_with_position.txt', self.unigram_inverted_index_with_positions)

    def save_position_to_file(self, file_name, sorted_inverted_index):
        with open(file_name, 'w') as f:
            for word in sorted_inverted_index.keys():
                f.write(word)
                f.write('= ')
                ...
                f.write(", ")
                for doc_id in sorted_inverted_list.keys():
                    tf = sorted_inverted_list[doc_id][0]
                    position_list = sorted_inverted_list[doc_id][1]
                    f.write("(")
                    f.write(doc_id)
                    f.write(",")
                    f.write(str(tf))
...
u.create_unigram_with_position()